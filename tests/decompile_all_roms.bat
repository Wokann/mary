...
target\debug\mary.exe decompile rom\fomteu.gba goodies\fomt_callables.mary.h --all --mary-c --symbols goodies\fomt_scripts_text.mary.sym --charmap charmap.txt -D MARY_FOMT_EU -o decompiled_text\fomt_eu
if errorlevel 1 goto :failed

echo [5/7] Decompiling FoMT DE as Mary-C...
target\debug\mary.exe decompile rom\fomtde.gba goodies\fomt_callables.mary.h --all --mary-c --symbols goodies\fomt_scripts_text.mary.sym --charmap charmap.txt -D MARY_FOMT_DE -o decompiled_text\fomt_de
if errorlevel 1 goto :failed
